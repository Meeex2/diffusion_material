...
# Test parameters
false_positive_rate_scheme = 0.1  # Scheme's target false positive rate
g_fixed = 1                       # Fixed g dimension
t_fixed = 3                        # Sparsity parameter
noise_rate_fixed = 0.1             # Channel noise rate
num_trials = 100                   # Trials per vector size
sizes = [6, 12, 24, 48, 64, 96, 128, 192, 256]  # Tested sizes
num_test_bits = int(np.ceil(np.log2(1 / false_positive_rate_scheme)))  # =4

results = []

for n in sizes:
    k = 0 + g_fixed + num_test_bits  # message_length=0
    r = n - k
    
    # Skip sizes where scheme parameters are invalid
    if k > n or r <= 0 or binom(n, t_fixed) < 1:
        print(f"Skipping n={n} (k={k}, r={r})")
        continue
    ...
    try:
        ...
        encoding_key, decoding_key = KeyGen(
            n, 
            message_length=0, 
            false_positive_rate=false_positive_rate_scheme,
            t=t_fixed,
            g=g_fixed,
            r=r,
            noise_rate=noise_rate_fixed
        )
        ...
        for _ in range(num_trials):
            # Test watermarked vector
            s_watermarked = Encode(encoding_key)
            if Detect(decoding_key, s_watermarked):
                tp += 1
                
            # Test unwatermarked vector (random signs)
            s_unwatermarked = torch.sign(torch.randn(n, dtype=torch.double))
            if Detect(decoding_key, s_unwatermarked):
                fp += 1
                
        tpr = tp / num_trials
        fpr = fp / num_trials
        results.append((n, tpr, fpr))
...
# Plot results
if results:
    n_vals, tpr_vals, fpr_vals = zip(*results)
    
    plt.figure(figsize=(10, 6))
    plt.plot(n_vals, tpr_vals, 'o-', label='TPR (Watermarked Detection)')
    plt.plot(n_vals, fpr_vals, 's-', label='FPR (Unwatermarked False Alarm)')
    plt.axhline(false_positive_rate_scheme, color='r', linestyle='--', 
                label=f'Target FPR ({false_positive_rate_scheme})')
    
    plt.xscale('log')
    plt.xlabel('Vector Size (n)')
    plt.ylabel('Rate')
    plt.title('Zero-Bit Watermark Detection Performance')
    plt.legend()
    plt.grid(True)
    plt.show()
else:
